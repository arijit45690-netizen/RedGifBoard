        ...
        currentDownloadJob = serviceScope.launch {
            ...
            try {
                val cacheFile = withContext(Dispatchers.IO) {
                    ...
                    if (!file.exists()) {
                        // ⚡ Bolt Optimization: Use Glide to fetch from cache instead of downloading again.
                        // Since Glide already loads and displays the GIF in the grid, it's cached on disk.
                        // This eliminates the ~1-2s network download delay when sending a GIF.
                        val cachedFile = Glide.with(applicationContext)
                            .asFile()
                            .load(gif.urls.sd)
                            .submit()
                            .get()
