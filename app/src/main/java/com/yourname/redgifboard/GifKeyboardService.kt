        ...
        currentDownloadJob = serviceScope.launch {
            ...
            try {
                val cacheFile = withContext(Dispatchers.IO) {
                    ...
                    if (!file.exists()) {
                        // ⚡ Bolt Optimization: Use Glide's disk cache instead of downloading again
                        // The GifAdapter already downloaded this GIF to show it in the grid.
                        // By using Glide.downloadOnly(), we instantly get the cached file,
                        // saving data and making sending instantaneous.
                        val glideFile = Glide.with(applicationContext)
                            .downloadOnly()
                            .load(gif.urls.sd)
                            .submit()
                            .get()

                        // Glide gives us a file in its cache, but we need it with a .gif extension
                        // for the FileProvider to share it correctly with other apps.
