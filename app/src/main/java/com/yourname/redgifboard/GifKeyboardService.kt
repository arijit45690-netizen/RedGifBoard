        ...
        currentDownloadJob = serviceScope.launch {
            ...
            try {
                val cacheFile = withContext(Dispatchers.IO) {
                    ...
                    if (!file.exists()) {
                        // ⚡ Bolt Optimization: Use Glide to fetch the cached file instead of downloading it again
                        // The GifAdapter already loaded this via Glide, so it will be served instantly from disk cache
                        ...
                        glideCachedFile.copyTo(file, overwrite = true)
                    }
