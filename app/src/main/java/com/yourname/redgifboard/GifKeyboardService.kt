            ...
            try {
                val cacheFile = withContext(Dispatchers.IO) {
                    ...
                    if (!file.exists()) {
                        ...
                        // Reuse Glide's disk cache instead of downloading the GIF again.
                        // The GIF is already cached from rendering in the grid,
                        // so this is a near-instant disk read instead of a 1-2s network call.
                        val cachedGlideFile = com.bumptech.glide.Glide.with(this@GifKeyboardService)
                            .asFile()
                            .load(gif.urls.sd)
                            .submit()
                            .get()

                        cachedGlideFile.copyTo(file, overwrite = true)
                    }
